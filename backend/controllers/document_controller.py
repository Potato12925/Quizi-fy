...
from services.document_service import create_document, delete_document, get_document_by_id, get_documents, update_document

router = APIRouter(prefix="/documents", tags=["Documents"])


@router.post("", summary="Create document")
async def post_document(payload: DocumentCreateRequest, _: CurrentUser = Depends(require_roles("admin"))):
    try:
...
async def get_document_list(page: int = Query(default=1, ge=1), limit: int = Query(default=20, ge=1, le=100), _: CurrentUser = Depends(require_roles("admin"))):
    try:
...
async def get_document_detail(record_id: int, _: CurrentUser = Depends(require_roles("admin"))):
    try:
...
async def put_document(record_id: int, payload: DocumentUpdateRequest, _: CurrentUser = Depends(require_roles("admin"))):
    try:
...
async def delete_document_route(record_id: int, _: CurrentUser = Depends(require_roles("admin"))):
    try:
