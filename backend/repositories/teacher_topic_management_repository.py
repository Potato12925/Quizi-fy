...
async def list_teacher_documents_with_subjects(teacher_id: int) -> list[dict]:
    ...
    response = await asyncio.to_thread(
        ...
        .select("document_id,subject_id,title,subjects(subject_id,subject_name)")
...
async def find_teacher_document(document_id: int, teacher_id: int) -> dict | None:
    ...
    response = await asyncio.to_thread(
        lambda: supabase.table("documents")
        .select("document_id,teacher_id,title,subject_id")
...
async def find_topic_by_name(topic_name: str) -> dict | None:
    supabase = SupabaseManager.get_client()
    response = await asyncio.to_thread(
        lambda: supabase.table("topics")
        .select("topic_id,topic_name,updated_at")
...
async def create_topic(topic_name: str) -> dict:
    supabase = SupabaseManager.get_client()
    response = await asyncio.to_thread(
        lambda: supabase.table("topics")
        .insert({"topic_name": topic_name})
        .execute()
    )
    rows = response.data or []
    if not rows:
