...
SELECT_FIELDS = "topic_id,subject_id,topic_name,description,created_at,updated_at,deleted_at"
...
async def list_topics(page: int, limit: int, subject_id: int | None = None) -> tuple[list[dict], int]:
    supabase = SupabaseManager.get_client()
    start = (page - 1) * limit
    end = start + limit - 1
...
async def list_topics_by_subject_ids(page: int, limit: int, subject_ids: list[int]) -> tuple[list[dict], int]:
    if not subject_ids:
        return [], 0
    supabase = SupabaseManager.get_client()
    start = (page - 1) * limit
