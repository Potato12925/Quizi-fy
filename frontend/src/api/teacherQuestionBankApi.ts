...
export const getTeacherQuestionBank = async (filters: QuestionBankFilters): Promise<{ items: TeacherQuestionBankItem[]; meta: Record<string, unknown> | null }> => {
...
export const createTeacherManualQuestion = async (payload: ManualQuestionPayloadV2): Promise<TeacherQuestionBankItem> => {
  const res = await api.post<ApiEnvelope<TeacherQuestionBankItem>>('/teacher/question-bank/manual', payload);
  return res.data;
};

export const updateTeacherQuestion = async (questionId: number, payload: ManualQuestionPayloadV2): Promise<TeacherQuestionBankItem> => {
  const res = await api.put<ApiEnvelope<TeacherQuestionBankItem>>(`/teacher/question-bank/${questionId}`, payload);
  return res.data;
};

export const updateTeacherQuestionStatus = async (questionId: number, status: 'draft' | 'approved' | 'inactive' | 'rejected'): Promise<void> => {
  await api.patch<ApiEnvelope<{ question_id: number; status: string }>>(`/teacher/question-bank/${questionId}/status`, { status });
};
