...
import { getResources, uploadResource, updateResource, deleteResource, getSubjects, getTopics } from '@/api/teacherApi';
import type { TeacherResource, UploadResourcePayload, DbSubject, DbTopic } from '@/api/teacherApi';
...
export default function TeacherResourcesPage() {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const [data, subjectData, topicData] = await Promise.all([
        getResources(user.user_id),
        getSubjects(user.user_id),
        getTopics(user.user_id),
      ]);
      setResources(data);
  ...
  const handleOpenUpload = () => {
    ...
    const defaultTopicId = topics.find((t) => t.subject_id === defaultSubjectId)?.topic_id || 0;
    setModalMode('upload');
    setFormData({ title: '', subjectId: defaultSubjectId, topicId: defaultTopicId, description: '' });
  ...
  const handleOpenEdit = (res: TeacherResource) => {
    ...
    setFormData({
      title: res.name,
      subjectId: res.subjectId || subjects[0]?.subject_id || 0,
      topicId: res.topicId || 0,
  ...
  const topicOptions = topics.filter((topic) => topic.subject_id === formData.subjectId);
         ...
         <div className="flex flex-wrap flex-1 gap-4">
            <div className="relative flex-1 min-w-[240px]">
               ...
               <input 
                 ...
                 className="w-full py-3 pl-12 pr-6 text-xs font-bold transition-all border-none rounded-xl bg-slate-50 focus:ring-2 focus:ring-red-500/20" 
               />
            </div>
            <select 
              value={filterSubject}
         ...
         {filteredResources.length > 0 ? (
           filteredResources.map(res => (
             <div key={res.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-red-900/5 transition-all group relative overflow-hidden">
                <div className="flex items-start justify-between mb-6">
                   <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-sm ${
                         ...
                         res.name.toLowerCase().endsWith('.txt') ? 'bg-slate-50 text-slate-500' : 
                         ...
                         <span className="material-symbols-outlined">
                            {res.name.toLowerCase().endsWith('.pdf') ? 'picture_as_pdf' : 
                             res.name.toLowerCase().endsWith('.txt') ? 'article' : 
                             res.name.toLowerCase().endsWith('.pptx') ? 'slideshow' : 'description'}
                         </span>
                      </div>
                      <div>
                         <h4 className="mb-2 text-base font-black leading-none tracking-tight text-slate-800">{res.name}</h4>
                   ...
                   <div className="flex gap-2 transition-opacity opacity-0 group-hover:opacity-100">
                      <button 
                        ...
                        className="flex items-center justify-center w-10 h-10 transition-all rounded-xl bg-slate-50 text-slate-400 hover:bg-slate-900 hover:text-white"
                      >
                ...
                <div className="flex items-end justify-between pt-6 mt-6 border-t border-slate-50">
                   ...
                   <div className="flex items-center gap-4">
                      <div className="pr-4 text-right border-r border-slate-100">
                      ...
                      >
                         <span className="text-base material-symbols-outlined">auto_awesome</span> Tạo câu hỏi
                      </Link>
                   </div>
                </div>
             </div>
              ...
              <div className="p-10 sm:p-12">
                 <div className="flex items-center justify-between mb-10">
                    ...
                    <button 
                      ...
                      className="flex items-center justify-center w-12 h-12 transition-all rounded-2xl bg-slate-50 text-slate-400 hover:bg-slate-100"
                    >
                 ...
                 <form onSubmit={handleSubmit} className="space-y-8">
                      ...
                      >
                         ...
                         {selectedFile ? (
                           <div className="flex flex-col items-center">
                              <span className="mb-2 text-4xl material-symbols-outlined text-emerald-500">check_circle</span>
                              <p className="max-w-xs text-xs font-black truncate text-emerald-700">{selectedFile.name}</p>
                              <p className="text-[10px] text-emerald-500 font-bold uppercase mt-1">{(selectedFile.size / 1024 / 1024).toFixed(2)} MB</p>
                           </div>
                         ) : (
                           <div>
                              <span className="mb-2 text-4xl material-symbols-outlined text-slate-200">cloud_upload</span>
                              <p className="text-xs font-black tracking-widest uppercase text-slate-400">Kéo thả hoặc click để chọn file</p>
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                       ...
                       <div className="space-y-2">
                          ...
                          <select 
                            ...
                            onChange={e => {
                              ...
                              const firstTopicId = topics.find((t) => t.subject_id === subjectId)?.topic_id || 0;
                              setFormData({ ...formData, subjectId, topicId: firstTopicId });
                            }}
                            className="w-full p-4 text-xs font-bold border-none cursor-pointer rounded-2xl bg-slate-50 focus:ring-2 focus:ring-red-500/20"
                          >
                       ...
                       <div className="space-y-2">
                          ...
                          <select 
                            ...
                            className="w-full p-4 text-xs font-bold border-none cursor-pointer rounded-2xl bg-slate-50 focus:ring-2 focus:ring-red-500/20"
                          >
                             <option value={0}>Không chọn topic</option>
                    ...
                    <div className="space-y-2">
                       ...
                       <textarea 
                         ...
                         className="w-full p-4 text-xs font-bold border-none resize-none rounded-2xl bg-slate-50 focus:ring-2 focus:ring-red-500/20"
                       />
                    </div>
