  ...
  return (
    <div className="max-w-4xl space-y-10 animate-in fade-in duration-700 pb-12">
      ...
      <section className="bg-white rounded-[2.5rem] p-10 border border-slate-50 shadow-sm space-y-8">
        <div className="flex items-center gap-8 pb-8 border-b border-slate-50">
           ...
           <div className="space-y-1">
              <h3 className="text-xl font-black text-slate-900">TS. Nguyễn Văn A</h3>
              <p className="text-sm text-slate-400 font-bold uppercase tracking-tight">Giảng viên Khoa CNTT 1 • Học viện Công nghệ BCVT</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           <div className="space-y-2">
              ...
              <input type="text" defaultValue="Nguyễn Văn A" className="w-full px-6 py-4 bg-slate-50 border border-slate-50 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-red-500/5 focus:border-[#b20112]/20 transition-all" />
           </div>
           <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Học viện</label>
              <input type="email" defaultValue="a.nv@ptit.edu.vn" disabled className="w-full px-6 py-4 bg-slate-100 border border-slate-100 rounded-2xl text-sm font-bold text-slate-400 outline-none cursor-not-allowed" />
           </div>
        </div>
      </section>
           ...
           <div className="space-y-4">
              ...
              <div className="space-y-3">
                 {['Gemini 1.5 Pro (Khuyên dùng)', 'GPT-4o Integration', 'Claude 3.5 Sonnet'].map((model, idx) => (
                    <div key={idx} className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${idx === 0 ? 'bg-white/10 border-red-500/50' : 'bg-white/5 border-transparent hover:bg-white/10'}`}>
                       <span className="text-xs font-bold">{model}</span>
                       <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${idx === 0 ? 'border-red-500' : 'border-white/20'}`}>
                          {idx === 0 && <div className="w-2 h-2 bg-red-500 rounded-full"></div>}
                       </div>
                    </div>
                 ))}
              </div>
           ...
           <div className="space-y-6">
              <div className="space-y-4">
                 <div className="flex justify-between items-end">
                    ...
                    <span className="text-xs font-black text-red-400">0.7</span>
                 </div>
                 <div className="h-1.5 w-full bg-white/10 rounded-full relative">
                    <div className="absolute left-0 top-0 h-full w-[70%] bg-red-500 rounded-full"></div>
                    <div className="absolute left-[70%] top-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full shadow-xl shadow-red-500/20"></div>
                 </div>
              </div>
              <div className="p-6 bg-white/5 rounded-[2rem] border border-white/5 space-y-3">
                 <div className="flex items-center justify-between">
                    <span className="text-xs font-bold">Giải thích chi tiết đáp án</span>
                    <div className="w-10 h-5 bg-red-500 rounded-full relative cursor-pointer">
                       <div className="absolute right-1 top-1 w-3 h-3 bg-white rounded-full"></div>
                    </div>
                 </div>
                 <div className="flex items-center justify-between">
                    <span className="text-xs font-bold">Gợi ý chủ đề liên quan</span>
      ...
      <div className="flex justify-end gap-4">
         ...
         <button className="px-10 py-4 bg-[#b20112] text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-red-900/20 hover:scale-105 active:scale-95 transition-all">Lưu cài đặt</button>
      </div>
    </div>
  );
}
